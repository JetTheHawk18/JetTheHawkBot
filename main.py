...
async def help(ctx):
    ...
    embed.add_field(name="COMMANDS", value='j!help - This message\nj!info - Information about the bot\nj!jettheme - Links you to the Theme Song of Jet the Hawk\nj!say - Make the bot say your message\nj!esay - Make the bot say stuff in a embed', inline=False)
...
async def info(ctx):
    ...
    embed.add_field(name="STATUS", value=f'Jet the Hawk Bot\nMade with: discord.py\nMade by: Jet The Hawk Main#8960\n\nLatency: {round(client.latency * 1000)}ms\nServers I am in: {len(client.guilds)}', inline=False)
    embed.add_field(name="LINKS", value='INVITE THE BOT: https://discord.com/oauth2/authorize?client_id=786401680798908426&permissions=0&scope=bot', inline=False)
...
@client.command(aliases=["_chat", "askjet"])
...
async def playjetstheme(ctx):
    ...
    voice.play(discord.FFmpegPCMAudio('./jetstheme.mp3'))
...
async def celebrate(ctx):
    voice = await ctx.author.voice.channel.connect()
    voice.play(discord.FFmpegPCMAudio('./soniccdpass.mp3'))
